...
function get_db_Calendars () {
	...
	while($row = $rs_cals->fetch_assoc()){
		...
		$db_Calendars[$row['calendar_id']]['syncdate']		= $row['syncdate'];
	}
	$rs_cals->free();
...
function get_syncCals ($db_Calendars, $google_Calendars) {
	...
	foreach ($google_Calendars as $row_G) {
		if (isset($db_Calendars[$row_G['calendar_id']])) {
			...
			$title_array = explode(' ', $row_G['calendar_title']);
			$listing = str_replace('#', '', $title_array[0]);

			$sql_ins = "INSERT INTO vrs6_calendars (`calendar_id`, `calendar_title`, `listing_id`) VALUES ('".$row_G['calendar_id']."', '".$row_G['calendar_title']."', $listing)";
...
function get_google_deleted($cal_id) {
	...
	$sql = "SELECT * FROM vrs6_bookings WHERE event_id IS NOT NULL AND TRIM(event_id) <> '' AND booking_status!='0' AND (blank_booking=1 OR (blank_booking=0 AND booking_status = 0)) AND end_date >= NOW() AND calendar_id = '".$cal_id."'";
	$eventos = query_db($sql) ;
